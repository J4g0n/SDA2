 ...
 */
Couleur** toLeafMatrix(Arbq aq)
{
	unsigned int imsize = nat2uint(tc(aq));
	unsigned int i, ll, cc;
	Couleur** matrix = MALLOCN(Couleur*, imsize);
	for (i = 0; i < imsize; i++)
		matrix[i] = MALLOCN(Couleur, imsize);
...
{
	unsigned int imsize = nat2uint(tc(aq));
	Couleur** leaves = toLeafMatrix(aq);
	unsigned int ll, cc;
